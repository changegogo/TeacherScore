	...
	function schoolChoice(d) {
		$(".area").on("singleTap", function() {
			...
			for (var i = 0; i < arr.length; i++) {
				list += "<li>" + arr[i].sch + "</li>" + "<span style='display: none;'>" + arr[i].subcode + "</span>"+"<span style='display: none;'>" + arr[i].schID + "</span>";
				$("#schools").html(list);
				$("li").unbind();
				$("li").on("singleTap", function() {
					...
					temp.schID = $(this).next().next().html();
				})
			}
		});
	}
		...
		if (confirm('评论老师的类型: ' + typeValue)) {
			...
			if (temp.role_Level != 3) {
				...
				$.ajax({
					...
					method : "GET",
					traditional : true,
					url : "ClassAndTeacherServlet?schoolName=" + temp.sch_Name + "&majorName=" + temp.cus_Name + "&schoolId= " + temp.schID + "&role_Level=" + typeValue,
					timeout : 5000,
					success : function(data) {
						if (data.code == 200) {
